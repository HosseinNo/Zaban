...
/**
 * «این کار مالِ کسی است که کلاس را اداره می‌کند، نه کسی که در آن نشسته.»
 *
 * own_class() فقط *دسترسی به کلاس* را می‌سنجد، نه اینکه چند ردیف باید
 * برگردد. برای زبان‌آموز با دامنهٔ own، تنها می‌پرسد «ثبت‌نام فعال
 * داری؟» و بعد کد ادامه می‌دهد و کل فهرست کلاس را برمی‌گرداند.
 *
 * دو جا همین باعث نشتی شده بود: attendance.php|get وضعیت حضور و شمار
 * غیبتِ همهٔ هم‌کلاسی‌ها را می‌داد، و assignments.php|status وضعیت
 * تحویل و *نمرهٔ* همه را. در پنل، فقط مدرس این‌ها را صدا می‌زند — ولی
 * پنهان‌بودن دکمه امنیت نیست؛ هر کسی با یک کوکی معتبر می‌توانست
 * مستقیم درخواست بزند.
 *
 * پس هر جا خروجی «دربارهٔ کل کلاس» است، این را صدا بزنید.
 */
function require_class_staff(string $perm): void
{
    if (scope_rank(perm_scope($perm) ?? 'own') < scope_rank('own_classes')) {
        fail(403, 'forbidden', 'این اطلاعات فقط برای مدرس و مدیر کلاس است.');
    }
}
