...
/*
 * ساعت جلسه به وقت تهران است، نه UTC.
 *
 * start_time همان چیزی است که مدیر در فرم کلاس تایپ می‌کند — «۱۸:۰۰»
 * یعنی شش عصرِ آموزشگاه — و در همهٔ پنل‌ها بی‌تبدیل نمایش داده می‌شود.
 * ولی پنجرهٔ ورود زبان‌آموز آن را با پسوند ' UTC' می‌خواند. ایران
 * سه ساعت و نیم جلوتر از گرینویچ است، پس لینک کلاسِ ۱۸:۰۰ ساعت ۲۱:۱۵
 * به وقت تهران باز می‌شد — وقتی کلاس نود دقیقه‌ای ساعت ۱۹:۳۰ تمام شده
 * بود. مدرس و مدیر لینک را همیشه می‌بینند، پس کلاس شروع می‌شد و فقط
 * زبان‌آموزها «هنوز زود است» می‌گرفتند.
 *
 * تاریخ‌ها هنوز میلادی ذخیره می‌شوند (بند P.2)؛ فقط تفسیرِ «ساعت دیواری
 * آموزشگاه» درست شد. منطقهٔ زمانی ثابت است چون محصول فقط برای ایران است
 * و ایران از ۱۴۰۱ ساعت تابستانی ندارد (پایگاه tz همین را می‌داند).
 */
const SCHOOL_TZ = 'Asia/Tehran';

function session_start_ts(string $date, string $time): int
{
    try {
        $dt = new DateTimeImmutable($date . ' ' . $time . ':00', new DateTimeZone(SCHOOL_TZ));
...
/** «امروزِ» آموزشگاه — بین نیمه‌شب تا ۳:۳۰ بامداد، gmdate هنوز دیروز است */
function school_today(): string
{
    return (new DateTimeImmutable('now', new DateTimeZone(SCHOOL_TZ)))->format('Y-m-d');
}
